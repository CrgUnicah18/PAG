...
@section('content')
    <div class="container">
        ...
        <div class="flex justify-end mb-2 p-3 relative right-7">
            <a href="{{ route('admin.configuracion.tipos-permisos.create') }}" class="btn btn-primary">
                Crear permiso
            </a>
        </div>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                @if(isset($tiposPermiso) && $tiposPermiso->count() > 0)
                    @foreach ($tiposPermiso as $tipoPermiso)
                        <tr>
                            ...
                            <td>{{ $tipoPermiso->es_vacacion ? '✅ Sí' : '❌ No' }}</td>
                            <td>
                ...
                @else
                    <tr>
                        <td colspan="5" class="text-center">No hay tipos de permisos registrados.</td>
                    </tr>
                @endif
            </tbody>
        </table>
